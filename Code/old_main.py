...
imgDir = str(BASE_DIR / "Images" / "chair1.jpg") # Image path if camera is not available
...
# Movement phase state and control
...
def fusion_thread():
    ...
    while running:
        ...
        if roi_median_depth is not None:
            cv2.putText(final_combined, f"Target Depth: {roi_median_depth:.1f}", (w_y + 50, h_y + 150), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        ...
        # --- Movement phase trigger ---
        # Begin movement phase when we have detections and metric depth and movement is not active.
        try:
            # Use globals for movement control
            global movement_phase_active, movement_target_queue, movement_target
        except Exception:
            movement_phase_active = False

        # Trigger when there's at least one box, a roi median depth, and no active movement
        if boxes and roi_median_depth is not None and not movement_phase_active:
            # Build a simple movement payload
            class_name = None
            try:
                class_name = names[classes[0]] if classes else None
            except Exception:
                class_name = str(classes[0]) if classes else None

            movement_payload = {
                "box": boxes[0],
                "class_id": classes[0] if classes else None,
                "class_name": class_name,
                "roi_median_depth": float(roi_median_depth),
                "median_depth": float(median_depth),
                "timestamp": time.time(),
            }

            try:
                movement_target_queue.put_nowait(movement_payload)
                movement_target = movement_payload
                logging.info(f"Enqueued movement target: {class_name}")
            except queue.Full:
                try:
                    movement_target_queue.get_nowait()
                    movement_target_queue.put_nowait(movement_payload)
                    movement_target = movement_payload
                    logging.info("Replaced old movement target with new one")
                except queue.Empty:
                    pass
...
# Movement controller thread (structure only)
def movement_controller_thread():
    """Waits for movement targets, sets movement phase active, and waits for completion.
    This is only the control structure; the actual movement logic should be implemented
    where indicated (e.g., by another module or hardware controller).
    """
    global running, movement_phase_active, movement_target, movement_target_queue, movement_done_event

    logging.info("Movement controller thread started")

    while running:
        try:
            payload = movement_target_queue.get(timeout=0.1)
        except queue.Empty:
            continue

        # Start the movement phase (structure only)
        movement_phase_active = True
        movement_target = payload
        movement_done_event.clear()
        logging.info(f"Movement phase started for target: {payload.get('class_name')} with depth {payload.get('roi_median_depth')}")

        # WAIT: In a real system you'd call the actuator / planner here and wait for completion.
        # For now we wait for either an explicit external signal (movement_done_event.set()) or a timeout.
        movement_timeout = 30.0  # seconds - placeholder safety timeout
        start = time.time()
        while running and not movement_done_event.is_set() and (time.time() - start) < movement_timeout:
            time.sleep(0.1)

        if movement_done_event.is_set():
            logging.info("Movement phase ended by external signal")
        else:
            logging.info("Movement phase ended by timeout")

        # Clear state so system can be re-triggered
        movement_phase_active = False
        movement_target = None
        movement_done_event.clear()

    logging.info("Movement controller thread exiting")
...
if __name__ == "__main__":
    ...
    t4.start()
    # Movement controller thread (structure only)
    t5 = threading.Thread(target=movement_controller_thread)
    t5.start()
    ...
    while running:
        try:
            ...
            # Overlay movement-phase status if active
            try:
                if movement_phase_active:
                    cv2.putText(combined_frame, "Movement Phase ACTIVE", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 255), 2)
                    if movement_target is not None and isinstance(movement_target, dict):
                        md = movement_target.get("roi_median_depth")
                        if md is not None:
                            cv2.putText(combined_frame, f"Target depth: {md:.2f}", (10, 125), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
            except Exception:
                pass
        ...
        if key == ord('q'):
            running = False
            # If a movement is active, signal done so controller can exit cleanly
            movement_done_event.set()
            break
        elif key == ord('c'):
            # Signal that movement phase is complete (for testing / manual end)
            movement_done_event.set()
            logging.info("User requested end of movement phase (pressed 'c')")
